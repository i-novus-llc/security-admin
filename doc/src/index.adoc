...
|access.organization-persist-mode=rdm
|rdm,crud
|Режим работы с организациями 1)Синхронизация организаций через нси и выключенные CRUD(rdm) 2)Синхронизация через нси выключена и работают CRUD(crud)

|access.keycloak.synchronizeEnabled=false
|boolean
|Включение автоматической синхронизации пользователей
