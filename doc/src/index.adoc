...
+
[listing]
@Bean @ConditionalOnExpression("T(org.springframework.util.StringUtils).isEmpty('${access.jwt.signing-key:}') && T(org.springframework.util.StringUtils).isEmpty('${access.jwt.verifier-key:}')") public KeyPair keyPairFromJKS() { return new KeyStoreKeyFactory(new ClassPathResource("keystore/gateway.jks"), properties.getPassword().toCharArray()).getKeyPair("gateway");
}
...
+
[listing]
access.auth.keystore.key-id=hYyfxVDkchN9wXuLLjSLe5kQ2EUrW4Mr access.auth.keystore.password=youKeyStorePassword

Вариант 2 pem формат: :::
* Установите openssl
* Выполните последовательно команды
+
[listing]
openssl req -newkey rsa:2048 -new -nodes -x509 -days 3650 -keyout privateKey.pem -out cert.pem
+
[listing]
openssl rsa -in privateKey.pem -pubout -out pubKey.pem

* В файлах privateKey.pem и pubKey.pem будут лежать закрытый и открытый ключ соответственно.
* Перейдите в модуль `auth-gateway` и в `application.properties` укажите корректные настройки ключей:
+
[listing]
access.jwt.signing-key=-----BEGIN PRIVATE KEY-----.....-----END PRIVATE KEY----- access.jwt.verifier-key=-----BEGIN PUBLIC KEY-----.....-----END PUBLIC KEY-----
...
транзитивные зависимости от spring-security-oauth2 и зависимость `security-auth-common`.
`security-auth-common` содержит в себе реализацию интерфейса `PrincipalExtractor` с помощью которой
...
|access.esia.signing-key
|string
|Ключ для подписи запросов к ЕСИА

|access.esia.certificate
|string
|Сертификат от ЕСИА
...
permissions,
systems
|Наполнение токена ролями, пермишенами и системами.
Указать одно или несколько значений через запятую.
...
|access.keycloak.synchronize-enabled
|boolean
|Включение автоматической синхронизации пользователей
