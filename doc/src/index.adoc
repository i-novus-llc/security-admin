...
* В файлах privateKey.pem и pubKey.pem будут лежать закрытый и открытый ключ соответсвенно.
* Перейдите в модуль `auth-gateway` и в `application.properties` укажите корректные настройки ключей:
+
[listing]
access.jwt.signing-key=-----BEGIN PRIVATE KEY-----.....-----END PRIVATE KEY-----
access.jwt.verifier-key=-----BEGIN PUBLIC KEY-----.....-----END PUBLIC KEY-----
